...
@Suppress("unused")
fun <T : BaseDoc<*>> String?.toStringIdSet(): Set<StringId<T>>? = this?.split(',')?.map { StringId<T>(it) }?.toSet()

@Suppress("unused")
fun <T : BaseDoc<*>> String?.toIntIdSet(): Set<IntId<T>>? = this?.split(',')?.map { IntId<T>(it.toInt()) }?.toSet()
